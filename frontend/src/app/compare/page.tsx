...
export default async function ComparePage({ searchParams }: { searchParams: CompareSearchParams }) {
  ...
  return <div className="min-h-screen pb-24">
    <section className="border-b border-[#dfa6b51f] bg-[#fff1f4] py-10 text-center md:py-14"><div className="container-page"><p className="eyebrow mb-4">COMPARE POWER</p><h1 className="text-balance font-myeongjo text-[32px] leading-tight md:text-5xl">같은 조건에서 나란히 보기</h1><p className="mt-4 text-sm leading-7 text-[#786c63]">{analysisProfile.skinType} · {analysisProfile.concerns.join(" · ")} {savedProfile ? "내 프로필" : "예시 프로필"}로 두 제품을 같은 기준에서 분석해요.</p><p className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/70 px-3 py-1.5 text-[10px] font-bold text-[#98495d]"><ShieldCheck size={13} /> 광고나 제휴 여부는 비교 결과에 반영하지 않아요</p>{!savedProfile && <div><Link href="/profile" className="mt-3 inline-flex min-h-11 items-center text-xs font-semibold text-[#9b4a45]">내 피부 프로필로 다시 비교하기</Link></div>}</div></section>
    <section className="container-page py-8 md:py-16">
      ...
      <div className="mt-7 rounded-[24px] border border-[#a54f4922] bg-[#f1e2db73] p-5 md:flex md:items-center md:gap-6 md:rounded-[26px] md:p-8"><span className="seal h-12 w-12 shrink-0 font-myeongjo text-xl">解</span><div className="mt-4 min-w-0 md:mt-0"><div className="flex items-center gap-2 text-xs font-bold text-[#994944]"><Sparkles size={14}/> 조건별 해석</div><p className="mt-2 font-myeongjo text-lg leading-8">{isTie ? <><strong>두 제품의 종합 적합도가 같아요.</strong> 사용감과 주효능을 보고 선택해보세요.</> : <><strong>{recommendation.name}</strong>의 현재 조건 적합도가 더 높아요. {strengths.length > 0 ? `${strengths.join(" · ")} 항목에서 더 유리하게 계산됐어요.` : "세부 근거를 확인한 뒤 선택해 보세요."}</>}</p><p className="mt-2 text-xs leading-5 text-[#82736a]">이 결과는 구매 권유가 아니라 입력한 피부 조건을 기준으로 한 정보 비교예요.</p></div><Link href={`/products/${recommendation.id}`} className="line-btn mt-5 w-full shrink-0 md:ml-auto md:mt-0 md:w-auto">판정서 자세히 보기</Link></div>
