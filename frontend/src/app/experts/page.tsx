...
export default async function ExpertsPage() {
  ...
  return (
    <div className="min-h-screen pb-24">
      <section className="relative overflow-hidden border-b border-[#edcbd3] bg-[radial-gradient(circle_at_78%_20%,rgba(246,193,205,.42),transparent_28%),linear-gradient(145deg,#fff_0%,#fff4f6_62%,#fde8ed_100%)] py-14 md:py-24">
        <div className="container-page relative grid gap-10 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
          <div>
            ...
            <h1 className="font-myeongjo text-4xl font-semibold leading-tight md:text-6xl">피부 고민에<br /><span className="text-[#a24f64]">검증된 시선</span>을 더해요</h1>
            <p className="mt-6 max-w-xl text-sm leading-7 text-[#75666b] md:text-base">의사·전문의·근무지 인증을 구분해 보여주고, 화장품과 성분에 관한 답변을 한곳에 모았습니다.</p>
            <div className="mt-8 flex flex-wrap gap-3"><Link href="/questions" className="ink-btn"><MessageCircleQuestion size={18} /> 전문가에게 질문하기</Link><Link href="/experts/ranking" className="line-btn"><Award size={18} /> 활동 랭킹</Link></div>
          </div>
          <div className="paper-card rounded-[34px] p-6 md:p-8">
      ...
      <section className="border-y border-[#eed5db] bg-[#fff7f9] py-12 md:py-18">
      ...
      <section className="container-page py-12 md:py-18">
        ...
        <div className="mt-10 flex flex-col items-center rounded-[28px] bg-[#3b3033] px-6 py-8 text-center text-white md:flex-row md:justify-between md:text-left"><div><p className="flex items-center justify-center gap-2 text-xs font-bold tracking-[.14em] text-[#efb9c5] md:justify-start"><Sparkles size={15} /> EXPERT JOIN</p><h3 className="mt-2 font-myeongjo text-2xl font-bold">화력의 신뢰를 함께 만들어주세요</h3><p className="mt-2 text-sm text-white/65">의료진 인증 후 전문 답변 활동을 시작할 수 있어요.</p></div><Link href="/experts/apply" className="mt-5 inline-flex min-h-12 items-center rounded-full bg-white px-6 text-sm font-bold text-[#403236] md:mt-0">전문가 인증 신청</Link></div>
      </section>
    </div>
  );
}
