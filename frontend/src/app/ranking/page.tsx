...
export default async function RankingPage({ searchParams }: { searchParams: RankingSearchParams }) {
  ...
  return <div className="min-h-screen pb-24">
    <section className="border-b border-[#dfa6b51f] bg-[#fff0f3] py-12 text-[#382b30] md:py-20"><div className="container-page"><div className="grid items-end gap-7 md:grid-cols-[1fr_auto] md:gap-8"><div><p className="text-xs font-bold tracking-[.18em] text-[#ad566d]">HWA:RYEOK RANKING</p><h1 className="mt-4 text-balance font-myeongjo text-[34px] font-medium leading-tight sm:text-4xl md:text-5xl">판매량보다,<br />내 피부에 잘 맞는 순위</h1><p className="mt-5 max-w-lg text-sm leading-7 text-[#7d6870]">선택한 피부 타입에 맞춰 Spring Boot가 화력 점수를 다시 계산해 보여드려요.</p></div><div className="rounded-2xl border border-white/90 bg-white/72 p-4 text-sm shadow-[0_14px_36px_rgba(164,82,104,.09)] backdrop-blur-xl sm:p-5"><div className="flex items-center gap-2 text-[#a54f63]"><Sparkles size={16}/><strong>현재 랭킹 기준</strong></div><p className="mt-2 text-xs leading-6 text-[#7d6870]">피부 타입 · 제품 효능<br />기본 화력 점수</p></div></div></div></section>

    <nav aria-label="피부 타입별 랭킹" className="sticky top-[72px] z-30 max-w-full overflow-hidden border-b border-[#dfa6b51f] bg-white/82 backdrop-blur-xl md:top-[74px]"><div className="container-page scrollbar-hide flex max-w-full snap-x snap-mandatory scroll-px-1 gap-2 overflow-x-auto py-3.5 sm:py-4">{tabs.map((item) => <Link key={item.value} href={item.value === "나의 피부" ? "/ranking" : `/ranking?tab=${encodeURIComponent(item.value)}`} aria-current={selectedTab.value === item.value ? "page" : undefined} className={`shrink-0 snap-start rounded-full px-4 py-2.5 text-xs transition ${selectedTab.value === item.value ? "bg-[#a54f63] text-white" : "border border-[#e3b1bd44] bg-[#fff8fa] text-[#706068]"}`}>{item.label}</Link>)}</div></nav>
