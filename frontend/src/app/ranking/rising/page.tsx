...
import { getRisingProductRanking } from "@/lib/api";
import { getFavoriteViewState } from "@/lib/auth-session";
import { catalogRankingHref, firstSearchValue, RankingPagination, requestedRankingPage, type CatalogRankingSearch } from "../_components/catalog-ranking-controls";
...
export default async function RisingRankingPage({ searchParams }: { searchParams: Promise<CatalogRankingSearch> }) {
  ...
  const [initialData, favorites] = await Promise.all([
    getRisingProductRanking({ category: requestedCategory || undefined, page, size: 12 }),
    getFavoriteViewState(),
  ]);
  let data = initialData;
  if (data.category && !data.categories.some((item) => item.name === data.category)) {
    data = await getRisingProductRanking({ page, size: 12 });
  }
  if (data.totalPages > 0 && page >= data.totalPages) {
    data = await getRisingProductRanking({ category: data.category ?? undefined, page: data.totalPages - 1, size: 12 });
  }
  const category = data.category ?? "";
  const filters = { category, ingredientId: "", minReviewScore: "", minFirepowerScore: "" };
  ...
  return <div className="container-page pb-24 pt-4 sm:pt-7">
    ...
    <section aria-labelledby="rising-products-heading">
      ...
      <RankingFilterSheet variant="rising" basePath={basePath} resultCount={data.totalElements} axes={[
        { id: "category", param: "category", label: "제품 유형", shortLabel: "종류", value: category, options: [{ value: "", label: "전체 상품" }, ...data.categories.map((item) => ({ value: item.name, label: item.name, count: item.productCount }))] },
      ]} />
      {data.content.length ? <div className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-5 xl:grid-cols-4">
        ...
        <Link href={page > 0 ? catalogRankingHref(basePath, filters) : category ? basePath : "/products"} className="line-btn mt-6">{page > 0 ? "첫 페이지 보기" : category ? "전체 급상승 보기" : "제품 둘러보기"}<ArrowRight size={14} /></Link>
      </div>}
