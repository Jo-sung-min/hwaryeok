  ...
  return <article className={styles.product}>
    <Link href={`/products/${encodeURIComponent(product.id)}`} className={styles.productLink}>
      ...
      <div className={styles.productText}>
        ...
        {review && <p className={styles.review}>{review.score === null ? `리뷰 ${review.count}개 · 점수 집계 중` : <>{growth ? "최근 7일 리뷰" : "리뷰"} <strong>{review.score.toFixed(1)}</strong> / 100 <span>({review.count})</span></>}</p>}
