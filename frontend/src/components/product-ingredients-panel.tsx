...
function IngredientRow({ ingredient }: { ingredient: ProductIngredient }) {
  ...
  return <Link href={`/ingredients/${ingredient.id}`} className="group flex items-start justify-between gap-4 rounded-[22px] border border-[#74513f17] bg-[#fffaf291] p-5 transition hover:border-[#9f705e4a]">
    <div className="flex min-w-0 gap-4"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${caution ? "bg-[#dca38d25] text-[#ad705c]" : "bg-[#8c9b8020] text-[#71806b]"}`}>{caution ? <TriangleAlert size={16}/> : <Check size={16}/>}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-myeongjo text-lg font-semibold">{ingredient.name}</h3>{ingredient.concentrationNote && <span className="rounded-full bg-[#8c796b12] px-2 py-1 text-[9px] text-[#79695f]">{ingredient.concentrationNote}</span>}{regulations.length > 0 && <span className="inline-flex items-center gap-1 rounded-full border border-[#d8a6b459] bg-[#fff0f3] px-2 py-1 text-[9px] font-bold text-[#934d60]"><ShieldCheck size={11} /> 공식 사용조건 {regulations.length}건</span>}</div><p className="mt-1 text-xs font-semibold text-[#9a6556]">{ingredient.role}</p><p className="mt-3 line-clamp-2 text-xs leading-6 text-[#71655d]">{caution ? ingredient.caution : ingredient.description}</p>{regulations.length > 0 && <p className="mt-2 text-[10px] leading-5 text-[#8f6671]">{Array.from(new Set(regulations.map((item) => item.country).filter(Boolean))).join(" · ") || "식약처 원문"}의 제품 유형·농도 조건을 확인하세요.</p>}</div></div><ChevronRight size={17} className="mt-2 shrink-0 text-[#9a8275] transition group-hover:translate-x-0.5"/>
  </Link>;
}
